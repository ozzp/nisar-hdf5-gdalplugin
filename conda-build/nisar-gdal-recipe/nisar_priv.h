                                      ...
                                      void *op_data)
{
    ...
    if( EQUAL(attr_name, "DIMENSION_LIST") || EQUAL(attr_name, "REFERENCE_LIST") || EQUAL(attr_name, "CLASS") || EQUAL(attr_name, "NAME") ) {
    ...
    if (attr_id < 0) return 0; // Attribute likely doesn't exist, not an error
    ...
    if (n_points == 1)
    {
        if (type_class == H5T_STRING) {
            ...
            else { value_str = "(read error float)"; }
        }
        // Handle Compound Type like GetGDALDataType
            ...
            GDALDataType eBaseType = GDT_Unknown; // To store the base type if complex
            ...
            if (H5Tget_nmembers(native_type) == 2) {
                ...
                if (hRealType >= 0 && hImagType >= 0) {
                    if (H5Tequal(hRealType, hImagType) > 0) { // Members must be same type
                        ...
                        if (isReal && isImaginary) {
                            bIsComplex = true; // Flag that it matches complex structure
                            // Determine the base GDAL type
                            if (H5Tequal(hRealType, H5T_NATIVE_FLOAT) > 0) eBaseType = GDT_Float32;
                            else if (H5Tequal(hRealType, H5T_NATIVE_DOUBLE) > 0) eBaseType = GDT_Float64;
                            else if (H5Tequal(hRealType, H5T_NATIVE_SHORT) > 0) eBaseType = GDT_Int16;
                            else if (H5Tequal(hRealType, H5T_NATIVE_INT) > 0) eBaseType = GDT_Int32;
            ...
            if (bIsComplex && eBaseType != GDT_Unknown) {
                if (eBaseType == GDT_Float32) {
                    ...
                    hid_t mem_type = H5Tcreate(H5T_COMPOUND, sizeof(ComplexFloatAttr));
                    H5Tinsert(mem_type, "r", HOFFSET(ComplexFloatAttr, r), H5T_NATIVE_FLOAT);
                    H5Tinsert(mem_type, "i", HOFFSET(ComplexFloatAttr, i), H5T_NATIVE_FLOAT);
                    if (H5Aread(attr_id, mem_type, &cfVal) >= 0) {
                         if (std::isnan(cfVal.r) || std::isnan(cfVal.i)) { value_str = "nan"; }
                         else { value_str = CPLSPrintf("%.10g + %.10gj", cfVal.r, cfVal.i); }
                    } else { value_str = "(read error complex float)"; }
                    H5Tclose(mem_type);
                } else if (eBaseType == GDT_Float64) {
                    ComplexDoubleAttr cdVal;
                    hid_t mem_type = H5Tcreate(H5T_COMPOUND, sizeof(ComplexDoubleAttr));
                    H5Tinsert(mem_type, "r", HOFFSET(ComplexDoubleAttr, r), H5T_NATIVE_DOUBLE);
                    H5Tinsert(mem_type, "i", HOFFSET(ComplexDoubleAttr, i), H5T_NATIVE_DOUBLE);
                    if (H5Aread(attr_id, mem_type, &cdVal) >= 0) {
                        if (std::isnan(cdVal.r) || std::isnan(cdVal.i)) { value_str = "nan"; }
                        else { value_str = CPLSPrintf("%.18g + %.18gj", cdVal.r, cdVal.i); }
                    } else { value_str = "(read error complex double)"; }
                    H5Tclose(mem_type);
                    ...
                    } else { value_str = "(read error complex int16)"; }
                    H5Tclose(mem_type);
                    ...
                    H5Tinsert(mem_type, "i", HOFFSET(ComplexInt32Attr, i), H5T_NATIVE_INT);
                     if (H5Aread(attr_id, mem_type, &ciVal) >= 0) {
                        value_str = CPLSPrintf("%d + %dj", ciVal.r, ciVal.i);
                    } else { value_str = "(read error complex int32)"; }
                    H5Tclose(mem_type);
                }
                // Add else if for other base types if needed
                else {
                    value_str = "(unhandled complex base type)"; // Should not happen if eBaseType != GDT_Unknown
                }
            } else {
